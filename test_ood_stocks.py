...
ood_stock_fns = ["acbi.us.txt", "hscz.us.txt", "qvcb.us.txt", "qsr.us.txt"]
...
for batch_id, samples in enumerate(loader): # iterate over batches
    ...
    avg_loss += loss.item()
    if batch_id % 50 == 0:
        print("(test) Batch {}/{} -- Loss: {} -- Pred: {}, True: {}".format(batch_id+1, len(loader), loss.item(), pred.item(), labels.item()))
avg_loss /= len(loader)
